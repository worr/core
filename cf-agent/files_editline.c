...

    {
        ...
        for (;;)
        {
            ...
            if (strncmp(buffer, "[%CFEngine", strlen("[%CFEngine")) == 0)
            {
                ...
                np = PromiseTypeAppendPromise(tp, promiser, (Rval) { NULL, RVAL_TYPE_NOPROMISEE, false }, context, NULL);
            ...
            else
            {
                if (IsDefinedClass(ctx, context))
                {
                    ...
                    else
                    {
                        ...
                        np = PromiseTypeAppendPromise(tp, buffer, (Rval) { NULL, RVAL_TYPE_NOPROMISEE, false }, context, NULL);
                        np->offset.line = lineno;
                        PromiseAppendConstraint(np, "insert_type", RvalNew("preserve_all_lines", RVAL_TYPE_SCALAR), false);
                    }
                }
